...
public class PullImobskyOfferService implements PullOfferServiceTask {

	protected static Logger logger = Logger.getLogger(PullImobskyOfferService.class);

	private Advertisers advertisers;

	public PullImobskyOfferService(Advertisers advertisers) {

		this.advertisers = advertisers;
	}

	/**
	 * 拉取PullImobskyOfferService广告信息，并入临时表中
	 * 
					...
					for (int i = 0; i < offersArray.size(); i++) {
						try {
							...
							if (offerItem != null) {
								...
								String kpi = offerItem.getString(("desc"));
								Float payout = offerItem.getFloat(("payout"));
								Integer cap = offerItem.getInteger(("cap_daily"));
								String pkg = offerItem.getString("app_pack_name");
								String country = offerItem.getString("countries");
	...
	public static void main(String[] args) throws ClientProtocolException, IOException {
		List<AdvertisersOffer> advertisersOfferList = new ArrayList<AdvertisersOffer>();
		String apiurl = "http://api.imobsky.com/v2/offer/pull?token=b66e9e8f351f4482a1ca3b024eeef4ee";
		String str = HttpUtil.sendGet(apiurl);
		JSONObject jsonPullObject = JSON.parseObject(str);
		JSONArray offersArray = jsonPullObject.getJSONArray("data");
		if (offersArray != null && offersArray.size() > 0) {
