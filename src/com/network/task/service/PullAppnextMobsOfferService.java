...
public class PullAppnextMobsOfferService implements PullOfferServiceTask{
	
	protected static Logger logger = Logger.getLogger(PullAppnextMobsOfferService.class);
	
	private Advertisers advertisers;
	
	public PullAppnextMobsOfferService(Advertisers advertisers){
	...
	public void run(){
		...
		synchronized(this){
			...
			logger.info("doPull Appnext Offer begin := " + new Date());
...
//				}
				...
				JSONArray offersArray = jsonPullObject.getJSONArray("apps");
...
//							}
							...
							String offer_model = item.getString("revenueType");// "CPI"
...
//							SONObject targeting =  JSON.parseObject(item.getString("targeting"));
							...
							//countriesArray=item.getString("countries");
							
//							String paltfrom=targeting.getString("platform");
							String os_str =null; 
							String app_id = item.getString("androidPackage");// "com.gtarcade.lod"
							if (app_id!=null &&!"".equals(app_id)) {
								os_str = "0";
							} else {
								app_id = item.getString("iphonePackage");
								if (app_id!=null &&!"".equals(app_id)) {
									os_str = "1";
								} else {
									continue;
...
//							String category = item.getString("appcategory");
							String icon = item.getString("urlImg");// "http://cdn.clickky.biz/s…Xzzyg=w3840_400x400.png"
...
//							}
							...
							String link = item.getString("urlApp");// "http://cpactions.com/api/v1.0/clk/track/proxy?ad_id=104573764&site_id=31466&response_id=59b8a7bd6cc66"
							...
							if (os_str.equals("0")) { //android
								...
								link=link+"&packageId=com.hsgame.FightingKing_KungfuClash.gp";
								link=link+"&d={gaid}&subid={sub_affid}";
							} else if (os_str.equals("1")) { //ios
								//link=link.replace("{gaid}", "");
								//link=link.replace("{android}", "");
								link=link+"&packageId=1146128499";
								link=link+"&d={idfa}&subid={sub_affid}";

							}
							
							Float payout = item.getFloat("revenueRate");// 0.31
...
//							}
	...
	public static void main(String[] args)  {
		...
		tmp.setApiurl("http://admin.appnext.com/offerApi.aspx?pimp=1&id={apikey}&city=1&ua=1&sub_accounts=1");
		//tmp.setApikey("b72a0d89-232a-46ea-b613-7762691e0a01");//android
		//7b613f43-9e09-4295-a09f-301e22f8f08a
		tmp.setApikey("7b613f43-9e09-4295-a09f-301e22f8f08a");
		tmp.setId(25L);
		
		PullAppnextMobsOfferService mmm=new PullAppnextMobsOfferService(tmp);
		mmm.run();
	}
