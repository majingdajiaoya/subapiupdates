...
public class PullBaiduSmartLinkOfferService implements PullOfferServiceTask{
	
	protected static Logger logger = Logger.getLogger(PullBaiduSmartLinkOfferService.class);
	
	private Advertisers advertisers;
	
	public PullBaiduSmartLinkOfferService(Advertisers advertisers){
	...
	public void run(){
		...
		synchronized(this){
			...
			logger.info("doPull BaiduSmartLink Offer begin := " + new Date());
			...
			if(adList != null
				...
				for(int i = 0; i < adList.size(); i++){
					...
					if(item != null){
						...
						JSONArray banner =item.getJSONArray("banner");
						...
						if (banner!=null&&banner.size()>0) {
							for (int j=0;j<banner.size();j++) {
								...
								JSONObject creativesitem = banner.getJSONObject(j);
								if (arrayList.size() <3) {
									Map jsonObject = new HashMap();
						            jsonObject.put("url",creativesitem.getString("url"));
						            jsonObject.put("size",creativesitem.get("size") );
						...
						advertisersOffer.setSmartlink_type(0);// Smartlink类型,类型的
	...
	public static void main(String[] args)  {
		...
		tmp.setApiurl("http://duclick.baidu.com/click/getOffers?aff_id=627&api_key={apikey}&page={page}&limit=500");//{apikey}
		tmp.setApikey("d048c6c54f583175e753343c4a30e867");
		tmp.setId(45L);
		
		PullBaiduSmartLinkOfferService mmm=new PullBaiduSmartLinkOfferService(tmp);
		mmm.run();
	}
