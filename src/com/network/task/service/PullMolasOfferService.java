...
public class PullMolasOfferService implements PullOfferServiceTask{
	
	protected static Logger logger = Logger.getLogger(PullMolasOfferService.class);
	
	private Advertisers advertisers;
	
	public PullMolasOfferService(Advertisers advertisers){
	...
	public void run(){
		...
		synchronized(this){
			...
			logger.info("doPull Mola Offer begin := " + new Date());
				...
				if(offersArray != null
					...
					for(int i = 0; i < offersArray.size(); i++){
						...
						if(item != null){
							...
							Integer roffer_id = item.getInteger("ad_id");// 104573764
...
//							}
							...
							Integer offer_modelint = item.getInteger("adtype");// "CPI"
...
//							SONObject targeting =  JSON.parseObject(item.getString("targeting"));
							String countriesArray=item.getString("c2code");
							
//							String paltfrom=targeting.getString("platform");
							Integer os_strint =item.getInteger("os"); 
							String os_str=null;
							if (os_strint == 0) {
								os_str = "1";
							} else if (os_strint == 1) {
...
//							}
							...
							String app_id = item.getString("pkg");// "com.gtarcade.lod"
							...
							String description = item.getString("title");
...
//							}
							...
							Integer caps_daily_remaining = 3000;
...
//							}
							// "need_gaid":0,
							Integer need_gaid=item.getInteger("need_gaid"); //是否需要gaid的
...
//							}
	...
	public static void main(String[] args)  {
		...
		//http://ads.mobimax.info/afs.do?sid=107374&token=708f2a&cat=1&adn=10
		tmp.setApiurl("http://ads.mobimax.info/afs.do?sid=107374&token={apikey}&cat=1&adn=10");
		tmp.setApikey("708f2a");
		tmp.setId(25L);
		
		PullMolasOfferService mmm=new PullMolasOfferService(tmp);
		mmm.run();
	}
