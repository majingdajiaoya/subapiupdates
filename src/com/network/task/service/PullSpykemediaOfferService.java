...
public class PullSpykemediaOfferService implements PullOfferServiceTask {

	protected static Logger logger = Logger.getLogger(PullSpykemediaOfferService.class);

	private Advertisers advertisers;

	public PullSpykemediaOfferService(Advertisers advertisers) {

		this.advertisers = advertisers;
	}

	/**
	 * 拉取PullSpykemediaOfferService广告信息，并入临时表中
	 * 
	...
	public void run() {
		...
		synchronized (this) {
			...
			logger.info("doPull 3Point14Mobs Offer begin := " + new Date());
				...
				if (offersArray != null && offersArray.size() > 0) {
					...
					for (int i = 0; i < offersArray.size(); i++) {
						...
						if (item != null) {
							...
							String icon = item.getString("logo");// "http://cdn.PullSpykemediaOfferService.biz/s…Xzzyg=w3840_400x400.png"
							...
							if (roffer_id == null || payout == null || countriesArray == null || OverseaStringUtil.isBlank(link) || OverseaStringUtil.isBlank(offer_model)) {
	...
	public static void main(String[] args) {
		...
		tmp.setApiurl("http://api-uceurope.affise.com/3.0/partner/offers?api-key=5865b3a3954a55417badf53432937df9a91ae7a2&limit=500");
		tmp.setApikey("d509889799899a41038dee3df8ddd8e386f04f39");
		tmp.setId(1078L);
		PullSpykemediaOfferService mmm = new PullSpykemediaOfferService(tmp);
		mmm.run();
	}
