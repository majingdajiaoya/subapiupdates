...
public class PullMediuminOfferService implements PullOfferServiceTask {

	protected static Logger logger = Logger.getLogger(PullMediuminOfferService.class);

	private Advertisers advertisers;

	public PullMediuminOfferService(Advertisers advertisers) {

		this.advertisers = advertisers;
	}

	/**
	 * 拉取PullMediuminOfferService广告信息，并入临时表中
	 * 
					...
					for (int i = 0; i < offersArray.size(); i++) {
						try {
							...
							if (offerItem != null) {
								...
								String name = offerItem.getString(("offer_name"));
								String trackinglink = offerItem.getString(("tracking_link"));
								String preview_url = offerItem.getString(("preview_url"));
								String kpi = offerItem.getString(("kpi"));
								Float payout = offerItem.getFloat(("offer_price"));
								Integer cap = offerItem.getInteger(("daily_cap"));
								String pkg = offerItem.getString("package");
								String country = offerItem.getString("regions");
								String os = "0";
								if (offerItem.getString("os").toUpperCase().contains("ANDROID")) {
									os = "0";
								} else if (offerItem.getString("os").toUpperCase().contains("IOS")) {
									os = "1";
								} else {
									continue;
								}
	...
	public static void main(String[] args) throws ClientProtocolException, IOException {
		List<AdvertisersOffer> advertisersOfferList = new ArrayList<AdvertisersOffer>();
		String apiurl = "http://api.mediumin.com/offers/?aff_id=1091&aff_token=soDe8GUo5LShSCODKrY2JIKDtygVqj9F";
		String str = HttpUtil.sendGet(apiurl);
		JSONObject jsonPullObject = JSON.parseObject(str);
		JSONArray offersArray = jsonPullObject.getJSONArray("data");
		if (offersArray != null && offersArray.size() > 0) {
		...
		// tmp.setId(1069L);
		// PullMediuminOfferService mmm = new PullMediuminOfferService(tmp);
		// mmm.run();
	}
