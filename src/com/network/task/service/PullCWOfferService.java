...
public class PullCWOfferService implements PullOfferServiceTask {

	protected static Logger logger = Logger.getLogger(PullCWOfferService.class);

	private Advertisers advertisers;

	public PullCWOfferService(Advertisers advertisers) {
	...
	public void run() {
		...
		synchronized (this) {
			...
			try {
				...
				JSONArray offersArray = json.getJSONArray("datas");
				...
				if (offersArray != null && offersArray.size() > 0) {
					...
					for (int i = 0; i < offersArray.size(); i++) {
						try {
							...
							if (item != null) {
								JSONObject offerItem = offersArray
										.getJSONObject(i);
								String offerid = offerItem.getString("id");
								String pkg = offerItem.getString("app_pkg");
								String tracklink = offerItem
										.getString("click_url");
								String name = offerItem.getString("app_name");
								String icon = offerItem.getString("app_icon");
								Integer daily_cap = offerItem
										.getInteger("daily_cap");
								String previewlink = offerItem
										.getString("preview_url");
								String description = offerItem.getString("des");
								String countries = offerItem
										.getString("countries");
								...
								String platform = offerItem.getString(
