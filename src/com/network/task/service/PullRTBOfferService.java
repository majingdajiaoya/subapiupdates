...
public class PullRTBOfferService implements PullOfferServiceTask {

	protected static Logger logger = Logger.getLogger(PullRTBOfferService.class);

	private Advertisers advertisers;

	public PullRTBOfferService(Advertisers advertisers) {
					...
					for (int i = 0; i < offersArray.size(); i++) {
						try {
							...
							if (offerItem != null) {
								...
								if (previewlink.contains("google") || previewlink.contains("apple")) {
									...
									Integer cap = offerItem.getInteger(("daily_conversion_cap"));
									String description = offerItem.getString(("description"));
									String os = offerItem.getString("os");
									String icon = offerItem.getString("icon_url");
	...
	public static void main(String[] args) {
		...
		tmp.setApiurl("http://retail9.com/genxml.php?affid=322689");
		tmp.setApikey("key=tizv1j8w9u7ykjck&pubid=6393");
		tmp.setId(21L);
		
		PullRTBOfferService mmm=new PullRTBOfferService(tmp);
		mmm.run();
	}
