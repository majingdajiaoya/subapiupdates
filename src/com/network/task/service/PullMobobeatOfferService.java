...
public class PullMobobeatOfferService implements PullOfferServiceTask {

	protected static Logger logger = Logger.getLogger(PullMobobeatOfferService.class);

	private Advertisers advertisers;

	public PullMobobeatOfferService(Advertisers advertisers) {
	...
	public void run() {
		...
		synchronized (this) {
			...
			try {
				...
				JSONArray offersArray = JSON.parseArray(result);
					...
					for (int i = 0; i < offersArray.size(); i++) {
						try {
							...
							if (item != null) {
								...
								String pkg = item.getString("packageid");
								...
								String icon = item.getString("url_image");
								Integer cap = item.getInteger("cap");
								String platform = item.getString("devices");
								String os_str = null;
