...
public class PullWeveOfferService implements PullOfferServiceTask{
	
	protected static Logger logger = Logger.getLogger(PullWeveOfferService.class);
	
	private Advertisers advertisers;
	
	public PullWeveOfferService(Advertisers advertisers){
	...
	public void run(){
		...
		synchronized(this){
			...
			logger.info("doPull WeveOffer Offer begin := " + new Date());
			...
			try {
				...
				String status = jsonPullObject.getString("code");
				Integer recordsTotal = jsonPullObject.getInteger("offer_numbers");
...
//							}
							...
							String app_id = item.getString("package_id");// "com.gtarcade.lod"
...
//							}
							...
							if(!OverseaStringUtil.isBlank(caps_daily_remaining_str)){
								if("-1".equals(caps_daily_remaining_str)){
...
//							}
							...
							Float payout = item.getFloat("offer_price");// 0.31
...
//							}
							...
							String images = item.getString("creatives");
			               ...
			               // "x":"http://sm-campaign.s3-us-west-2.amazonaws.com/resource/icons/source_1_prod_670_source_55_prod_708_59b23dc121a71.jpg"
							...
							if (images!= null&&!images.equals("{}")&&!images.equals("[]")) {
								try {
									...
									JSONArray creativesad= JSONArray.parseArray(images);
									if (creativesad!=null &&creativesad.size() >0) {
										for (int j=0;j<creativesad.size();j++) {
											JSONObject images_temp = creativesad.getJSONObject(j);
	...
	public static void main(String[] args)  {
		...
		tmp.setApiurl("http://offer.wevemob.com/api?userId=95&token={apikey}");
		tmp.setApikey("zYb0W0amU32vU7gn");
		tmp.setId(25L);
		
		PullWeveOfferService mmm=new PullWeveOfferService(tmp);
		mmm.run();
	}
