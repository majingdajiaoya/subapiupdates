...
public class PullYepadsCpasOfferService implements PullOfferServiceTask{
	
	protected static Logger logger = Logger.getLogger(PullYepadsCpasOfferService.class);
	
	private Advertisers advertisers;
	
	public PullYepadsCpasOfferService(Advertisers advertisers){
	...
	public void run(){
		...
		synchronized(this){
			...
			try {
				...
				if (offersArray2 != null&&offersArray2 != null) {
					...
					if (idsd.size()>0) {
						apiurl="http://api.pbydom.com/api-ext/v1/ag/campaigns/?limit=500&offset=0&status=08&vertical_id=5";
						...
						JSONArray offersArray = JSONArray.parseArray(str2);
						try {
							if(offersArray != null
									&& offersArray.size() > 0){
...
//										}
	...
	public static void main(String[] args)  {
		...
		tmp.setApiurl("http://api.pbydom.com/api-ext/v1/offers/?limit=500&offset=0&status=08&vertical_id=2,8");
		tmp.setApikey("044b3b8695d9e86768cd10a67f7c0c60827202b1");
		tmp.setId(49L);
		
		PullYepadsCpasOfferService mmm=new PullYepadsCpasOfferService(tmp);
		mmm.run();
	}
