...
public class PullClickkyOfferService implements PullOfferServiceTask{
	
	protected static Logger logger = Logger.getLogger(PullClickkyOfferService.class);
	
	private Advertisers advertisers;
	
	public PullClickkyOfferService(Advertisers advertisers){
	...
	public void run(){
		...
		synchronized(this){
			...
			logger.info("doPull Clickky Offer begin := " + new Date());
			...
			try {
				...
				if(available ==  null
					...
					logger.info("available or  count is empty. available:= " + available + " count:=" + count);
				...
				if(offersArray != null
					...
					for(int i = 0; i < offersArray.size(); i++){
						...
						if(item != null){
							...
							String offer_model = item.getString("offer_model");// "CPI"
							String traffic_type = item.getString("traffic_type");// "Non Incentive"
							
							Integer incetive = null;// 是否激励(0:非激励 1:激励)
							if("Non Incentive".equals(traffic_type)){
								
								incetive = 0;
							}
							else if("Incentive".equals(traffic_type)){
							...
							String target_url = item.getString("target_url");// "https://play.google.com/store/apps/details?id=com.gtarcade.lod"
							...
							String description = item.getString("description");// "Google Play Summer Sale …Lodsupport@gtarcade.com"
